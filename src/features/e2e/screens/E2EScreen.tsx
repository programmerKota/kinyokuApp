  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.backgroundTertiary }}>
      ...
      <View style={{ paddingHorizontal: 16, paddingTop: 12, paddingBottom: 4 }}>
        <RadioButton.Group onValueChange={(v) => setMode(v as 'light' | 'dark')} value={mode}>
          <List.Item
            title="ライト"
            description="明るいテーマ"
            left={() => <List.Icon icon="white-balance-sunny" />}
            right={() => <RadioButton value="light" />}
            style={{ backgroundColor: colors.backgroundSecondary, borderRadius: 8, marginBottom: 8 }}
          />
          <List.Item
            title="ダーク"
            description="暗いテーマ"
            ...
            style={{ backgroundColor: colors.backgroundSecondary, borderRadius: 8 }}
          />
        </RadioButton.Group>
      </View>
