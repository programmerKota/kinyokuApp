...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, ScrollView, Pressable, Platform, Image, KeyboardAvoidingView } from 'react-native';
...
import { signInWithEmailPassword, signUpWithEmailPassword, resetPassword, getRedirectTo, startOAuthFlow, updatePassword } from '@core/services/supabase/authService';
...
const AuthScreen: React.FC = () => {
  ...
  }, [emailErr, passErr, email, password, tab, agreeTermsTos, agreeTermsPrivacy]);
      ...
      if (tab === 'login') {
        try {
          ...
          if (/invalid login credentials/i.test(msg)) {
            ...
            setErrorMsg('メールアドレスの確認が完了していません。メールをご確認ください');
          } else {
            setErrorMsg('ログインに失敗しました。時間をおいて再度お試しください');
          }
          return;
        }
      } else {
        try {
          ...
          if (/user already registered/i.test(msg)) {
            setErrorMsg('このメールはすでに登録済みです。ログインをお試しください');
          } else {
  ...
  const loginDisabled = !!submitting || !(!emailErr && !passErr && !!email && !!password);
  ...
  const magicSignupDisabled = !!submitting || !!emailErr || !email || !(agreeTermsTos && agreeTermsPrivacy);
  const loginMailColor = magicLoginDisabled ? colors.gray500 : screenThemes.auth.accent;
  const signupMailColor = magicSignupDisabled ? colors.gray500 : screenThemes.auth.accent;
  ...
  const doReset = useCallback(async () => {
    ...
    try {
      ...
      setInfoMsg('パスワード再設定メールを送信しました。メールをご確認ください。');
    } catch (e: any) {
      ...
      if (/email rate limit|too many requests/i.test(msg)) {
        ...
        setErrorMsg('パスワード再設定に失敗しました。時間をおいて再度お試しください。');
      }
    } finally {
      setSubmitting(null);
    }
  ...
  const handleConfirmNewPassword = useCallback(async () => {
    ...
    if (e1) { setErrorMsg(e1); return; }
    if (newPass1 !== newPass2) { setErrorMsg('パスワードが一致しません'); return; }
    try {
      ...
      setErrorMsg('パスワードの更新に失敗しました。時間をおいて再度お試しください。');
    } finally {
        ...
        <View style={{ marginTop: spacing.lg }}>
          ...
          <View style={{ position: 'relative' }}>
            <TextInput
              ...
              style={[styles.input, (passTouched || triedSubmit) && passErr ? styles.inputError : null]}
            />
            <Pressable
              onPress={() => setShowPass((v) => !v)}
              accessibilityRole="button"
              accessibilityLabel={showPass ? 'パスワードを非表示' : 'パスワードを表示'}
  ...
  <View style={styles.linksRow}>
    <Pressable onPress={() => { try { void doReset(); } catch {} }}>
  ...
  {/* 送信ボタン（メール+パスワード） */}
  <DSButton
    title={tab === 'login' ? (submitting === 'login' ? 'ログイン中…' : 'ログイン') : (submitting === 'signup' ? '登録中…' : '新規登録')}
    onPress={() => { try { void submit(); } catch {} }}
  ...
  />
            ...
            <View style={{ marginTop: spacing.xl }}>
              ...
              {tab === 'login' ? (
                <Pressable
                  ...
                  style={({ pressed }) => [
                    styles.googleBtn,
                    { marginTop: spacing.md, opacity: pressed ? 0.92 : 1 }, magicLoginDisabled ? styles.googleBtnDisabled : styles.googleBtnEnabled,
                  ]}
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                    <Ionicons name="mail" size={18} color={loginMailColor} style={{ marginRight: 8 }} />
                    <Text style={[styles.googleText, magicLoginDisabled ? styles.googleTextDisabled : null]}>{submitting === 'magic' ? 'リンク送信中…' : 'メールリンクでログイン'}</Text>
                  </View>
                </Pressable>
              ) : (
                <Pressable
                  ...
                  style={({ pressed }) => [
                    styles.googleBtn,
                    { marginTop: spacing.md, opacity: pressed ? 0.92 : 1 }, magicSignupDisabled ? styles.googleBtnDisabled : styles.googleBtnEnabled,
                  ]}
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                    <Ionicons name="mail" size={18} color={signupMailColor} style={{ marginRight: 8 }} />
                    <Text style={[styles.googleText, magicSignupDisabled ? styles.googleTextDisabled : null]}>{submitting === 'magic' ? 'リンク送信中…' : 'メールリンクで登録'}</Text>
                  </View>
                </Pressable>
              )}
          ...
          <Modal
            ...
            onClose={() => { setShowResetModal(false); setErrorMsg(null); setInfoMsg(null); }}
            title="パスワード再設定"
            maxWidth={520}
          >
            <View style={{ gap: spacing.md }}>
              <Text style={{ color: colors.textSecondary }}>新しいパスワードを入力してください。</Text>
              <TextInput
              ...
              <DSButton
                ...
                onPress={() => { try { void handleConfirmNewPassword(); } catch {} }}
...
const createAuthStyles = (colors: any) => StyleSheet.create({
  ...
  errorBanner: { marginTop: spacing.sm, paddingVertical: 10, paddingHorizontal: spacing.md, backgroundColor: '#FEF2F2', borderRadius: 8, borderWidth: 1, borderColor: '#FCA5A5' },
