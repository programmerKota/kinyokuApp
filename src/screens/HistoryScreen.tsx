...
import { Challenge, Payment } from '../types';
import HistoryCard from '../components/HistoryCard';
import StatsCard from '../components/StatsCard';
import { StatsService } from '../services/statsService';
import { ChallengeService } from '../services/firestore';

type FilterType = 'all' | 'challenges' | 'payments';
...
const HistoryScreen: React.FC = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const challengesData = firestoreChallenges.map((challenge) => ({
                    ...
                    startedAt: challenge.startedAt instanceof Date
                        ? challenge.startedAt
                        : challenge.startedAt.toDate(),
                    completedAt: challenge.completedAt instanceof Date
                        ? challenge.completedAt
                        : challenge.completedAt?.toDate?.() || undefined,
                    failedAt: challenge.failedAt instanceof Date
                        ? challenge.failedAt
    ...
    const getFilteredData = () => {
        const allData = [
            ...challenges.map(c => ({ ...c, type: 'challenge' as const })),
            ...payments.map(p => ({ ...p, type: 'payment' as const })),
        ].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());

        switch (filter) {
            case 'challenges':
                return allData.filter(item => item.type === 'challenge');
            case 'payments':
                return allData.filter(item => item.type === 'payment');
            default:
                return allData;
        }
    };
    ...
    const getStats = () => {
        const totalPenaltyPaid = payments
            .filter(p => p.type === 'penalty' && p.status === 'completed')
            .reduce((sum, p) => sum + p.amount, 0);
        const totalPrizeEarned = payments
