...
import { SafeAreaView, StatusBar, StyleSheet, View, FlatList, TouchableOpacity, Text } from 'react-native';
...
import { BlockService } from '../services/firestore';
...
const BlockedUsersScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const { FirestoreUserService } = await import('../services/firestore');
                const list: SimpleUser[] = [];
                for (const id of blockedIds) {
                    try {
                        const u = await FirestoreUserService.getUserById(id);
                        if (u) list.push({ id, displayName: u.displayName || 'ユーザー', photoURL: u.photoURL });
                    } catch { }
                }
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity
                        activeOpacity={0.8}
                        style={styles.item}
                        onPress={() => navigateToUserDetail(navigation as any, item.id, item.displayName, item.photoURL)}
                    >
                        <UserProfileWithRank
