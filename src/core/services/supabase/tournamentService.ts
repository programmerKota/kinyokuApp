...
export class TournamentService {
  ...
  static subscribeToJoinRequests(
    ...
    const init = async () => {
      ...
      current = (data || []).map((row) => ({
        ...
        createdAt: toTs(
          (row as { createdAt?: string | null }).createdAt ?? null,
        ),
      ...
      const msg: import("../firestore/types").FirestoreTournamentMessage[] = [
        {
          ...
          authorAvatar:
            (row as { authorAvatar?: string | null }).authorAvatar ?? undefined,
    ...
    const applyChange = (
      ...
      const item = {
        ...
        maxParticipants: Number(
          (row as { maxParticipants: number }).maxParticipants,
        ),
        entryFee: Number((row as { entryFee: number }).entryFee),
        prizePool: Number((row as { prizePool: number }).prizePool),
        status: String((row as { status: string }).status) as
          | "upcoming"
          | "active"
          | "completed"
          | "cancelled",
        recruitmentOpen: Boolean(
          (row as { recruitmentOpen: boolean }).recruitmentOpen,
        ),
        startDate: toTs(
          (row as { startDate?: string | null }).startDate ?? null,
        ),
        ...
        updatedAt: toTs(
          (row as { updatedAt?: string | null }).updatedAt ?? null,
        ),
      ...
      (data || []).forEach((row) => {
        ...
        result[tid].push({
          ...
          leftAt: (row as { leftAt?: string | null }).leftAt
            ...
            : null,
          progressPercent: (row as { progressPercent?: number })
            .progressPercent,
  ...
  static subscribeToParticipants(
    ...
    const emit = () => callback([...current]);
    const sortAsc = (
      a: FirestoreTournamentParticipant,
      b: FirestoreTournamentParticipant,
    ) => {
      const at =
        a.joinedAt instanceof Date
          ? a.joinedAt.getTime()
          : Date.parse(String(a.joinedAt ?? "")) || 0;
      const bt =
        b.joinedAt instanceof Date
          ? b.joinedAt.getTime()
          : Date.parse(String(b.joinedAt ?? "")) || 0;
      return at - bt;
    };
    ...
    const init = async () => {
      ...
      current = (data || []).map((row) => ({
        ...
        userAvatar:
          (row as { userAvatar?: string | null }).userAvatar ?? undefined,
        status: (row as { status: FirestoreTournamentParticipant["status"] })
          .status,
