...
export function normalizeCommunityPosts<
  T extends { id: string; likes?: number; comments?: number },
>(list: T[]): T[] {
  return list.map((p) => ({
    ...p,
...
export function buildReplyCountMapFromPosts<
  T extends { id: string; comments?: number },
>(list: T[]): Map<string, number> {
...
export function toggleLikeInList<T extends { id: string; likes: number }>(
  ...
  return list.map((p) =>
    p.id === postId
      ? ({ ...p, likes: isLiked ? p.likes + 1 : Math.max(0, p.likes - 1) } as T)
      : p,
  );
}
