    ...
    return (
      <View style={styles.root}>
        ...
        <ScrollView style={styles.box} contentContainerStyle={{ padding: 12 }}>
          <Text selectable style={styles.message}>{String(error?.message || error)}</Text>
          {!!info?.componentStack && (
            <Text selectable style={styles.stack}>{info.componentStack}</Text>
          )}
        </ScrollView>
      </View>
    );
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: '#fff', paddingTop: 48, paddingHorizontal: 16 },
