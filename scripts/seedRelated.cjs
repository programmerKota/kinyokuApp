// CommonJS seeding script to avoid ts-node/ESM friction
// Mirrors scripts/seedRelated.ts functionality
...
async function seedChallenges(userId, k) {
  for (let i = 0; i < k; i++) {
...
async function seedPosts(userId, name, avatar) {
...
async function seedFollows(ids) {
  for (const a of ids) {
...
async function seedTournaments(ids) {
...
async function main() {
  ...
  console.log(`Seeding users=${userCount}, challengesPerUser=${chPerUser}`);
