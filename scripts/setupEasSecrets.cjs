...
function tryParseFirebaseConfigSnippet(snippet) {
  ...
  try {
    ...
    const map = {
      ...
      EXPO_PUBLIC_FIREBASE_MESSAGING_SENDER_ID:
        obj.messagingSenderId || obj.messagingSenderID,
...
async function createSecret(name, value) {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      ...
      console.warn(
        `[secrets] non-zero exit for ${name} (code ${code}). It may already exist. Continuing...`,
      );
      resolve();
    });
...
async function main() {
  ...
  console.log(
    "Make sure you are logged in: `npx eas whoami` should show your user.",
  );
