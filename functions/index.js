...
try {
  ...
  if (host && user && pass) {
    ...
    const csecure =
      cfg.smtp.secure !== undefined ? !!cfg.smtp.secure : cport === 465;
    transporter = nodemailer.createTransport({
      host: cfg.smtp.host,
      port: cport,
      secure: csecure,
    ...
    console.warn(
      "[functions] Mail transport not configured. Set SMTP_* or GMAIL_* env.",
    );
  }
} catch (e) {
...
exports.sendFeedbackEmail = functions.firestore
  ...
  .onCreate(async (snap) => {
    ...
    if (!transporter || !to) {
      console.warn(
        "[functions] Missing transporter or FEEDBACK_TO_EMAIL. Skipping email.",
      );
      return;
    }
...
exports.addDiaryForToday = functions.https.onCall(async (data, context) => {
  try {
    ...
    const startedAt =
      chData.startedAt && chData.startedAt.toDate
        ? chData.startedAt.toDate()
        : new Date(chData.startedAt);
