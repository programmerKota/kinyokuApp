...
serve(async (req) => {
  ...
  try {
    ...
    const appUserId: string | undefined =
      ev?.app_user_id || ev?.data?.app_user_id || payload?.app_user_id;
    const productId: string | undefined =
      ev?.product_id || ev?.data?.product_id || ev?.transaction?.product_id;
    const transactionId: string | undefined =
      ev?.transaction_id ||
      ev?.data?.transaction_id ||
      ev?.original_transaction_id;
    const platform: string | undefined =
      ev?.store || ev?.environment || ev?.platform;
    let amount: number | null = null;
    try {
      const price = ev?.price ?? ev?.data?.price;
      if (typeof price === "number") amount = Math.round(price);
      else if (typeof price?.amount_micros === "number")
        amount = Math.round(price.amount_micros / 1_000_000);
    } catch {}
    ...
    if (
      appUserId &&
      (successTypes.includes(t) ||
        cancelTypes.includes(t) ||
        refundTypes.includes(t))
    ) {
